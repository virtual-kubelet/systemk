...
Description = Foo
`
	deserialized, err := NewFile(contents)
	if err != nil {
		t.Fatalf("Unexpected error parsing unit %q: %v", contents, err)
	}
	...
	deserialized, err = NewFile(serialized)
	if err != nil {
